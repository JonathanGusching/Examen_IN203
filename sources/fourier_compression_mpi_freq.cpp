...
sparseSpectralComposition compressSpectralComposition(int begin, int end, int nbp, std::uint32_t width, std::uint32_t height, const std::complex<double>* plainCoefs, double tauxCompression )
{
    ...
    // on commence à begin et termine à end
...
int main(int nargs, char* argv[])
{
    ...
    auto sparse_loc = compressSpectralComposition(height_loc*rank, height_loc*(rank+1), nbp, width, height, fourier, taux);
    ...
    sparseSpectralComposition sparse=sparse_loc;

    int count=taux*height*width;
    MPI_Reduce(sparse_loc.coefficients.data(), sparse.coefficients.data(), count, MPI_DOUBLE, MPI_MAX, 0, globComm);
